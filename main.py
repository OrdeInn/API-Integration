...
def main():
    ...
    search_query = {
        ...
        'end_address': 'SFO',
        'mode': 'one_way',
        'pickup_datetime': '2023-09-05 04:30',
        'num_passengers': 2,
        'currency': 'USD',
        'campaign': 'Emirhan Oguz'
    }
    create_search_res = apiObj.createSearch(search_query)

    if (not create_search_res['success']):
        print('\nSomething went wrong on creating a search. Detalis:\n' 
    ...
    for result in search_result:
        ...
        for step in steps:
            ...
            if (provider['name'] == 'Dummy External Provider' and 
                price < cheapest_price):
                cheapest_test_result = result
                break

    if (cheapest_test_result == None):
    ...
    reservation_data = {
        ...
        'email': 'emirhanoguzsdfsdfdsf@gmail.com',
        ...
        'airline': 'AA',
        'flight_number': '123',
        'customer_special_instructions': 'Lorem ipsum dolar sit amet asdasdas',
    }

    book_reservation = apiObj.bookReservation(reservation_data)
    if (not book_reservation['success']):
        print('\nSomething went wrong on booking reservation. Detalis:\n' 
            + book_reservation['data'])
        exit()

    print('\nYour reservation is in progress...')
    reservation_result = apiObj.getBookingPoll(search_id)

    if (not reservation_result['success']):
        print('\nSomething went wrong on getting reservation result. Detalis:\n' 
    ...
    for reservation in reservation_list:
        ...
        cancel_booking = apiObj.cancelBooking(reservation_id)
        if (not cancel_booking['success']):
            print('\nSomething went wrong on cancelling your reservation. Detalis:\n' 
            + cancel_booking['data'])
            exit()
